...
describe('isWSL2', () => {
  ...
  it('returns true when WSL_INTEROP is set', () => {
    ...
    vi.stubEnv('WSL_INTEROP', '/run/interop')
    expect(isWSL2()).toBe(true)
  })
  ...
  it('returns true when /proc/version contains microsoft', () => {
    ...
    vi.mocked(readFileSync).mockReturnValue('Linux microsoft version')
    expect(isWSL2()).toBe(true)
  })

  it('returns false when /proc/version has no microsoft', () => {
    vi.stubEnv('WSL_DISTRO_NAME', '')
    vi.stubEnv('WSL_INTEROP', '')
    vi.mocked(readFileSync).mockReturnValue('Linux 5.15.0 Ubuntu')
