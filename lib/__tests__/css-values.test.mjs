...
// Parses an "oklch(L% C H)" string into [L, C, H] numbers for numeric
// comparison without depending on the exact formatting of trailing zeros.
...
describe('toOklch', () => {
  it('converts achromatic colors exactly (chroma and hue are zero)', () => {
    expect(toOklch('#fff')).toBe('oklch(100% 0 0)')
    expect(toOklch('#ffffff')).toBe('oklch(100% 0 0)')
    expect(toOklch('#000')).toBe('oklch(0% 0 0)')
  })

  it('converts a legacy hex literal to a reference OKLCH value', () => {
    const [L, C, H] = parseOklch(toOklch('#ff0000'))
    expect(L).toBeCloseTo(62.8, 0)
    expect(C).toBeCloseTo(0.258, 2)
    expect(H).toBeCloseTo(29.2, 0)
  })

  it('agrees across hex, rgb, and hsl spellings of the same color', () => {
    const hex = toOklch('#ff0000')
    expect(toOklch('rgb(255, 0, 0)')).toBe(hex)
    expect(toOklch('hsl(0, 100%, 50%)')).toBe(hex)
  })

  it('returns null for alpha, wide-gamut, and non-literal colors', () => {
    expect(toOklch('rgba(255, 0, 0, 0.5)')).toBeNull()
    expect(toOklch('#ff000080')).toBeNull()
    expect(toOklch('oklch(70% 0.1 250)')).toBeNull()
    expect(toOklch('red')).toBeNull()
    expect(toOklch('var(--brand)')).toBeNull()
    expect(toOklch('')).toBeNull()
    expect(toOklch(null)).toBeNull()
  })
})
