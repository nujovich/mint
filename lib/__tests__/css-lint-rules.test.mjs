...
describe('lintDeadCss', () => {
  it('detects an overridden property within a rule', () => {
    const css = '.a { color: red; color: blue; }'
    const result = lintDeadCss(css)
    expect(result.overriddenDeclarations).toHaveLength(1)
    const issue = result.overriddenDeclarations[0]
    expect(issue.selector).toBe('.a')
    expect(issue.property).toBe('color')
    expect(issue.deadValue).toBe('red')
    expect(issue.liveValue).toBe('blue')
    expect(issue.rule).toBe('overridden-property')
    expect(issue.severity).toBe('warning')
  })

  it('skips a vendor-prefixed fallback declaration', () => {
    const css = '.a { display: -webkit-box; display: flex; }'
    const result = lintDeadCss(css)
    expect(result.overriddenDeclarations).toHaveLength(0)
  })

  it('skips a literal value before a var() reference', () => {
    const css = '.a { color: #000; color: var(--brand); }'
    const result = lintDeadCss(css)
    expect(result.overriddenDeclarations).toHaveLength(0)
  })

  it('flags every overridden pair when a property is declared three times', () => {
    const css = '.a { margin: 0; margin: 1px; margin: 2px; }'
    const result = lintDeadCss(css)
    expect(result.overriddenDeclarations).toHaveLength(2)
    expect(result.overriddenDeclarations[0].deadValue).toBe('0')
    expect(result.overriddenDeclarations[1].deadValue).toBe('1px')
  })

  it('flags the universal selector as zero-specificity', () => {
    const result = lintDeadCss('* { box-sizing: border-box; }')
    expect(result.zeroSpecificitySelectors).toHaveLength(1)
    expect(result.zeroSpecificitySelectors[0].selector).toBe('*')
    expect(result.zeroSpecificitySelectors[0].rule).toBe('zero-specificity')
    expect(result.zeroSpecificitySelectors[0].severity).toBe('warning')
  })

  it('flags a :where()-only selector as zero-specificity', () => {
    const result = lintDeadCss(':where(.foo, .bar) { margin: 0; }')
    expect(result.zeroSpecificitySelectors).toHaveLength(1)
    expect(result.zeroSpecificitySelectors[0].selector).toBe(
      ':where(.foo, .bar)'
    )
  ...
  it('flags a combinator-only selector as zero-specificity', () => {
    const result = lintDeadCss('* + * { margin-top: 8px; }')
    expect(result.zeroSpecificitySelectors).toHaveLength(1)
    expect(result.zeroSpecificitySelectors[0].selector).toBe('* + *')
  })

  it('does not flag selectors with real specificity', () => {
    const css =
      '.foo { color: red; } div { color: red; } #id { color: red; } ' +
      '[hidden] { display: none; } a:hover { color: blue; }'
    const result = lintDeadCss(css)
    expect(result.zeroSpecificitySelectors).toHaveLength(0)
  })

  it('detects a selector declared more than once at top level', () => {
    const css = '.card { color: red; } .card { background: blue; }'
    const result = lintDeadCss(css)
    expect(result.duplicateSelectors).toHaveLength(1)
    expect(result.duplicateSelectors[0].selector).toBe('.card')
    expect(result.duplicateSelectors[0].rule).toBe('duplicate-selector')
    expect(result.duplicateSelectors[0].severity).toBe('suggestion')
  })

  it('does not flag the same selector inside a media query as duplicate', () => {
    const css =
      '.card { color: red; } ' +
      '@media (max-width: 600px) { .card { color: blue; } }'
    const result = lintDeadCss(css)
    expect(result.duplicateSelectors).toHaveLength(0)
  })

  it('detects overridden declarations inside a media query', () => {
    const css =
      '@media (max-width: 600px) { .card { color: red; color: blue; } }'
    const result = lintDeadCss(css)
    expect(result.overriddenDeclarations).toHaveLength(1)
    expect(result.overriddenDeclarations[0].selector).toBe('.card')
  })

  it('reports zero issues for clean CSS', () => {
    const css = '.text { color: red; font-size: 16px; }'
    const result = lintDeadCss(css)
    expect(result.totalIssues).toBe(0)
    expect(result.overriddenDeclarations).toEqual([])
    expect(result.zeroSpecificitySelectors).toEqual([])
    expect(result.duplicateSelectors).toEqual([])
  })

  it('returns zero issues for empty CSS', () => {
    const result = lintDeadCss('')
    expect(result.totalIssues).toBe(0)
  })

  it('aggregates totalIssues across all three signal types', () => {
    const css =
...
describe('formatDeadCssReport', () => {
  it('reports zero issues for clean CSS', () => {
    const report = formatDeadCssReport(lintDeadCss('.text { color: red; }'), 3)
    expect(report).toContain('static scan across 3 file(s)')
    expect(report).toContain('0 overridden declaration(s)')
    expect(report).toContain('0 zero-specificity selector(s)')
    expect(report).toContain('0 duplicate selector(s)')
    expect(report).toContain('No dead CSS found.')
  })

  it('reports an overridden declaration with dead and live values', () => {
    const report = formatDeadCssReport(
      lintDeadCss('.a { color: red; color: blue; }'),
      1
    )
    expect(report).toContain('1 overridden declaration(s)')
    expect(report).toContain('WARN  .a')
    expect(report).toContain('dead: red  ->  live: blue')
    expect(report).not.toContain('No dead CSS found.')
  })

  it('reports zero-specificity and duplicate selector findings', () => {
    const report = formatDeadCssReport(
      lintDeadCss('* { margin: 0; } .a { color: red; } .a { color: blue; }'),
      1
    )
    expect(report).toContain('1 zero-specificity selector(s)')
    expect(report).toContain('1 duplicate selector(s)')
    expect(report).toContain('WARN  *')
    expect(report).toContain('INFO  .a')
  })
})
