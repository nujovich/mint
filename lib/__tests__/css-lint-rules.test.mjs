...
describe('lintMotionAccessibility', () => {
  ...
  it('classifies the stylelint prefers-reduced-motion dataset correctly', () => {
    const css = readFileSync(
      new URL('../__fixtures__/prefers-reduced-motion.css', import.meta.url),
      'utf8'
    )
    const result = lintMotionAccessibility(css)

    // Every unwrapped motion declaration in the fixture is rejected, even
    // inside a `no-preference` frame (which is NOT a reduce wrapper).
    expect(result.unwrappedCount).toBe(9)
    expect(result.totalMotionDeclarations).toBe(9)
    expect(result.issues.map((i) => i.selector)).toEqual([
      '.spin',
      '.fade',
      '.menu',
      '.menu',
  ...
  it('does not flag the reduce-frame overrides or keyframe internals', () => {
    const css = readFileSync(
      new URL('../__fixtures__/prefers-reduced-motion.css', import.meta.url),
      'utf8'
    )
    const result = lintMotionAccessibility(css)
    const flagged = result.issues.map((i) => i.selector)

    // Each selector is flagged by its unwrapped top-level declaration only.
    // The `animation: none` / `transition: none` overrides that live inside
    // the `prefers-reduced-motion: reduce` frame are NOT flagged, so each
    // selector appears once, not twice.
    expect(flagged.filter((s) => s === '.spin')).toHaveLength(1)
    expect(flagged.filter((s) => s === '.fade')).toHaveLength(1)
    expect(flagged.filter((s) => s === '.hero')).toHaveLength(1)

    // The keyframes blocks add no issues: `.slide` appears once (its own
    // `animation` reference), and the `@keyframes slide` / `@-webkit-keyframes`
    // `transform` steps are skipped.
    expect(flagged.filter((s) => s === '.slide')).toHaveLength(1)
    expect(flagged.filter((s) => s === '.card')).toHaveLength(1)
  })

  it('keeps the transform declaration from a reduce frame and keyframes out of scope', () => {
    const css = readFileSync(
      new URL('../__fixtures__/prefers-reduced-motion.css', import.meta.url),
      'utf8'
    )
    const result = lintMotionAccessibility(css)
    const transformIssues = result.issues.filter(
      (i) => i.property === 'transform'
    )

    // The only unwrapped top-level transform is `.card`; the keyframe
    // `transform` steps live inside @keyframes and are skipped.
    expect(transformIssues).toHaveLength(1)
    expect(transformIssues[0].selector).toBe('.card')
    expect(transformIssues[0].value).toBe('translateX(24px) rotate(3deg)')
  })
})
