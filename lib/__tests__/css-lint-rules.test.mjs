...
describe('auditColorSpaces', () => {
  it('classifies color literals by space and computes the adoption rate', () => {
    const css = `
      :root {
        --brand: #1976d2;
      ...
      .card {
        ...
        background: hsl(207, 79%, 46%);
        border-color: color(display-p3 1 0.5 0);
      }
    `
    const audit = auditColorSpaces(css)
    expect(audit.legacyCount).toBe(3) // hex, rgb, hsl
    expect(audit.wideGamutCount).toBe(2) // oklch, display-p3
    expect(audit.samples).toHaveLength(5)
    expect(audit.adoptionRate).toBeCloseTo(2 / 5)
  })

  it('lists only legacy colors as migration candidates', () => {
    const css =
      '.a { color: #fff; } .b { color: oklch(0.5 0.1 200); } .c { background: rgb(1, 2, 3); }'
    const audit = auditColorSpaces(css)
    expect(audit.legacyCandidates.map((s) => s.format).sort()).toEqual([
      'hex',
      'rgb',
    ])
    expect(audit.legacyCandidates.every((s) => s.category === 'legacy')).toBe(
      true
    )
  })

  it('returns zero counts for CSS without color literals', () => {
    const audit = auditColorSpaces('.a { display: flex; }')
    expect(audit.legacyCount).toBe(0)
    expect(audit.wideGamutCount).toBe(0)
    expect(audit.adoptionRate).toBe(0)
    expect(audit.samples).toEqual([])
    expect(audit.legacyCandidates).toEqual([])
  })

  it('detects colors inside gradients and shadows, skipping keywords and var()', () => {
    const css = `
      .hero { background: linear-gradient(to right, #fff, oklch(90% 0.05 200)); }
      .card {
    ...
    `
    const audit = auditColorSpaces(css)
    const values = audit.samples.map((s) => s.value)
    expect(values).toContain('#fff')
    expect(values).toContain('oklch(90% 0.05 200)')
    expect(values).toContain('rgba(0, 0, 0, 0.5)')
    expect(values).not.toContain('red')
    expect(values).not.toContain('var(--brand)')
  })

  it('attributes each sample to its selector', () => {
    const css = '.a { color: #fff; } .b { background: oklch(0.6 0.1 100); }'
    const audit = auditColorSpaces(css)
    const hex = audit.samples.find((s) => s.format === 'hex')
    const oklch = audit.samples.find((s) => s.format === 'oklch')
    expect(hex.selector).toBe('.a')
    expect(oklch.selector).toBe('.b')
  })

  it('suggests an OKLCH equivalent for each convertible legacy color', () => {
    const css = `
      :root { --brand: #1976d2; --alpha: rgba(0, 0, 0, 0.5); }
      .card { color: rgb(25, 118, 210); background: hsl(207, 79%, 46%); }
    `
    const audit = auditColorSpaces(css)
    const legacy = audit.legacyCandidates
    // hex, rgb, hsl are convertible; the rgba sample has alpha < 1 and is not
    expect(legacy).toHaveLength(4)
    const opaque = legacy.filter((s) => !s.value.startsWith('rgba'))
    expect(opaque).toHaveLength(3)
    for (const s of opaque) {
      expect(s.oklch).toMatch(/^oklch\([\d.]+% [\d.]+ [\d.]+\)$/)
    }
    const alphaSample = legacy.find((s) => s.value.startsWith('rgba'))
    expect(alphaSample.oklch).toBeUndefined()
  })

  it('does not attach an OKLCH suggestion to wide-gamut samples', () => {
    const css = '.a { color: oklch(70% 0.1 250); }'
    const audit = auditColorSpaces(css)
    expect(audit.wideGamutCount).toBe(1)
    expect(audit.samples[0].oklch).toBeUndefined()
  })
})

describe('auditColorSpaces with a real design palette', () => {
  const palette = readFileSync(
    new URL('../__fixtures__/design-palettes.css', import.meta.url),
    'utf8'
  )

  it('classifies every color format in a multi-format palette', () => {
    const audit = auditColorSpaces(palette)
    const byFormat = audit.samples.reduce((acc, s) => {
  ...
  it('reports the legacy-vs-wide-gamut split and adoption rate', () => {
    const audit = auditColorSpaces(palette)
    expect(audit.legacyCount).toBe(8)
    expect(audit.wideGamutCount).toBe(4)
    expect(audit.samples).toHaveLength(12)
    expect(audit.adoptionRate).toBeCloseTo(4 / 12)
  })

  it('lists every legacy palette color as a candidate with an OKLCH suggestion', () => {
    const audit = auditColorSpaces(palette)
    expect(audit.legacyCandidates).toHaveLength(8)
    for (const s of audit.legacyCandidates) {
      expect(s.category).toBe('legacy')
      expect(s.oklch).toMatch(/^oklch\([\d.]+% [\d.]+ [\d.]+\)$/)
    }
  })

  it('leaves wide-gamut palette colors without a conversion suggestion', () => {
    const audit = auditColorSpaces(palette)
    const wide = audit.samples.filter((s) => s.category === 'wide-gamut')
    expect(wide).toHaveLength(4)
    for (const s of wide) {
      expect(s.oklch).toBeUndefined()
    }
  })

  it('attributes every palette color to its declaration selector', () => {
    const audit = auditColorSpaces(palette)
    expect(audit.samples.every((s) => s.selector === ':root')).toBe(true)
  })

  it('produces a numeric OKLCH suggestion for the brand anchor color', () => {
    const audit = auditColorSpaces(palette)
    const brand = audit.legacyCandidates.find((s) => s.value === '#1976d2')
    expect(brand).toBeDefined()
    expect(brand.oklch).toMatch(/^oklch\([\d.]+% [\d.]+ [\d.]+\)$/)
  })
})
