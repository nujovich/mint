...
describe('getDirtyFiles', () => {
  ...
  it('propagates a git status failure instead of reporting the tree clean', () => {
    vi.spyOn(cp, 'execFileSync').mockImplementation((_bin, args) => {
      if (args.includes('rev-parse')) return 'true\n'
      const e = new Error('fatal: ../x.css: outside repository')
      e.status = 128
      throw e
    })
    expect(() => getDirtyFiles(['../x.css'], '/repo')).toThrow(
      /outside repository/
    )
  })
})
