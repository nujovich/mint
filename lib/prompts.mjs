...
STEP 13 — CSS @PROPERTY TYPE CHECKING
Scan the CSS for @property at-rules. An @property registration defines a custom property's type contract via the syntax descriptor:
...
Build a table of every registration found: the property name (e.g. "--my-color"), its declared syntax (e.g. "<color>", "<length>", "<number>"), and its initial-value when present. Type-check values against these descriptors:
  - <color>: accepts hex, rgb(), rgba(), hsl(), hsla(), named colors, currentColor, transparent
  - <length>: accepts px, em, rem, vh, vw, vmin, vmax, ch, ex, cm, mm, in, pt, pc, 0, calc() expressions
  - <number>: accepts plain numbers (1, 0.5, 2.3), calc() that evaluates to a number
  - <integer>: accepts whole numbers (1, 0, -1), calc() that evaluates to an integer
  - <percentage>: accepts percentage values (50%, 100%)
  - <angle>: accepts deg, rad, grad, turn
  - <time>: accepts s, ms
  - <length-percentage>: accepts any length or percentage
  - <transform-function>: accepts transform functions (rotate(), scale(), translate(), etc.)
  - <custom-ident>: accepts any valid CSS identifier
  - <string>: accepts quoted strings
  - <url>: accepts url()
  - "*": universal — accepts any value, so no type checking is possible. Skip registrations declaring "*" entirely.

Then apply the three rules below. Every entry you record uses the same field set:
- "selector": the CSS selector containing the offending usage, or the at-rule written as "@property --name" for registration-level errors
- "property": the CSS property the value is assigned to, or "initial-value" for registration-level errors
- "rule": one of "invalid-initial-value", "fallback-type-mismatch", "property-type-mismatch"
- "severity": "warning" for the first two rules, "suggestion" for the third
- "reason": a specific message naming the declared syntax and the offending value
- "propertyName": the registered custom property (e.g. "--my-color")
- "declaredSyntax": the declared syntax string (e.g. "<color>")

13a. INVALID INITIAL-VALUE
The initial-value must itself parse as the declared syntax. When it does not, the browser rejects the whole registration and the property silently falls back to unregistered behaviour — losing both the type contract and the ability to transition. This is a hard error, not a preference. Also flag any registration that declares a non-universal syntax but omits initial-value, which is invalid per spec. Record these with "rule": "invalid-initial-value" and "severity": "warning", e.g. "reason": "Registered as \`<length>\` but \`initial-value: red\` is a \`<color>\` — the browser rejects this registration entirely".

13b. FALLBACK TYPE MISMATCH
Scan every var() usage of a registered property for a fallback whose type contradicts the declared syntax — var(--my-color, 14px) is registered as <color> but falls back to a <length>, so the fallback can never apply. Record these with "rule": "fallback-type-mismatch" and "severity": "warning".
...
- transitionDuration and transitionTimingFunction in theme.extend from typography.motion`,
  ...
  'unocss-config': (
    shared
  ) => `Generate a complete uno.config.ts for these design tokens, targeting the on-demand UnoCSS engine.${shared}

Requirements:
- Import { defineConfig, presetUno } from 'unocss' and export default defineConfig({ ... })
- theme.colors: all color tokens with full 50–900 scales as nested objects
- theme.fontFamily, theme.fontSize, theme.fontWeight, theme.lineHeight from the typography tokens
- theme.spacing, theme.borderRadius, theme.boxShadow extending the defaults
- theme.duration and theme.easing from typography.motion
- presets: [presetUno({ dark: 'class' })] for class-based dark mode, with a comment suggesting presetAttributify() and presetIcons() for users who want them
- A safelist array with the most common generated utilities (bg-, text-, border-, rounded-, shadow-)
- Add a short header comment noting it is a UnoCSS config`,
...
export const EXPORT_OUTPUT = {
  ...
  'unocss-config': { filename: 'uno.config', ext: 'ts' },
