...
For each @property registration found:
- Record the property name (e.g. "--my-color"), its declared syntax (e.g. "<color>", "<length>", "<number>"), and the initial-value if present.
- Then scan all var() usages of that registered property throughout the CSS. A var() usage is any appearance of var(--prop-name) in a property value.
- For each var() usage, check whether the assigned value or context is compatible with the declared syntax:
  ...
  - <url>: accepts url()
  - "*": universal — accepts any value (no type checking possible)

When the var(--prop-name) usage appears to contradict the declared syntax, record it. Examples:
- var(--my-color, 14px) — <color> with a <length> fallback → type mismatch
- var(--my-spacing) used inside background-color — suggests the property may be misused
- var(--my-size, red) — <length> with a <color> fallback → type mismatch

Focus on clear type mismatches: a <color> custom property receiving a length value, or a <length> receiving a color. Skip ambiguous cases where the type cannot be determined from the CSS alone (e.g., a var() whose value comes from a different stylesheet or JavaScript).

For each type mismatch found, record:
- "propertyName": the @property name (e.g. "--my-color")
- "declaredSyntax": the declared syntax string
- "location": selector and property name where the mismatch occurs
- "issue": description of the mismatch (e.g. "<color> property receives a <length> value")

Only report up to 8 propertyTypeIssues total. If no @property registrations exist in the CSS, record an empty array.</instructions>
...
<example>
{
  ...
  "propertyTypeIssues": [
    { "propertyName": "--my-color", "declaredSyntax": "<color>", "location": ".button >> background-color", "issue": "<color> property receives a <length> value (14px)" },
    { "propertyName": "--spacing-unit", "declaredSyntax": "<length>", "location": ".card >> color", "issue": "<length> property used in a color context, possible type mismatch" }
  ]
}
</example>
</output_format>`
