...
function hasDnsResultOrderOverride() {
...
// On WSL2, NAT routing can resolve api.anthropic.com to an IPv6 address that
// isn't reachable, causing fetch() to hang until ETIMEDOUT (issue #19).
// Force IPv4-first only on WSL2 so that non-WSL hosts keep their default
...
const DEBUG = process.env.MINT_DEBUG === '1' || process.env.MINT_DEBUG === 'true'

function debugLog(...args) {
  if (DEBUG) process.stderr.write('[mint:debug] ' + args.join(' ') + '\n')
}

function dnsLookupAll(host) {
  return new Promise((resolve) => {
    dnsLookup(host, { all: true, verbatim: false }, (err, addresses) => {
      if (err) resolve({ error: err.code || err.message })
...
function describeError(err) {
  ...
  if (err?.cause?.code) codes.push(`cause:${err.cause.code}`)
  for (const inner of err?.cause?.errors || []) {
    if (inner?.code) codes.push(`inner:${inner.code}`)
    if (inner?.address) codes.push(`addr:${inner.address}`)
    if (inner?.port) codes.push(`port:${inner.port}`)
  }
  return codes.join(' ') || err?.message || String(err)
}
