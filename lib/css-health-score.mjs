...
// ---------------------------------------------------------------------------
// Milestone 3 — Benchmarking against Project Wallace (2026 CSS analysis)
// ---------------------------------------------------------------------------

/**
 * Project Wallace 2026 CSS Selection benchmarks.
 *
 * Percentile reference points (p25 / p50 / p75 / p90 / p99) for each metric
 * computed by computeMetrics, derived from Wallace's annual analysis of
 * >100k production stylesheets (projectwallace.com/the-css-selection/2026).
 *
 * Grounded in published figures: e.g. median @layer adoption ~2.71% (so the
 * p50 of layerAdoption sits at 0.027), and median CSS complexity well above
 * the tidy range our thresholds treat as "good". These anchors let Mint tell
 * an operator how their stylesheet ranks against the real-world corpus.
 *
 * `lowerIsHealthier` marks whether a higher raw value means *less* healthy
 * CSS. Layer adoption is the lone inversion — more CSS inside @layer is better.
 */
export const WALLACE_2026 = {
  selectorsPerRule: {
    p25: 1.2,
    p50: 2.5,
    p75: 4.0,
  ...
  layerAdoption: {
    ...
    p90: 0.18,
    p99: 0.45,
    lowerIsHealthier: false,
  },
}

/**
 * Position of `value` within a percentile distribution, expressed as the
 * share of the corpus at or below it (0-100).
 *
 * The five anchor points are treated as the cumulative distribution of the
 * raw metric value; linear interpolation fills the gaps. Values beyond the
 * anchors are clamped to [0, 100].
 *
 * @param {number} value
 * @param {{p25:number,p50:number,p75:number,p90:number,p99:number}} dist
 * @returns {number}
 */
export function valuePercentile(value, dist) {
  const v = Number(value) || 0
  const anchors = [
    [0, 0],
    [25, dist.p25],
    ...
    [90, dist.p90],
    [99, dist.p99],
    // beyond p99, assume the tail extends ~50% further before saturating
    [100, dist.p99 * 1.5],
  ]
  if (v <= anchors[0][1]) return 0
  if (v >= anchors[anchors.length - 1][1]) return 100
  for (let i = 1; i < anchors.length; i++) {
    const [pLo, xLo] = anchors[i - 1]
    const [pHi, xHi] = anchors[i]
    if (v >= xLo && v <= xHi) {
      const t = (v - xLo) / (xHi - xLo)
      return round2(pLo + t * (pHi - pLo))
    }
  }
  return 100
}
...
/**
 * Compare computed CSS metrics against the Project Wallace 2026 corpus.
 *
 * For each metric, reports the analyzed value, the *health* percentile (share
 * of real sites that are at least as healthy), a verdict bucket, and a short
 * human-readable note. `layerAdoption` is inverted so that higher adoption
 * scores as healthier. An `overallPercentile` averages the per-metric health
 * standings, and a `summary` string gives a one-line readout.
 *
 * @param {object} metrics - Output of computeMetrics
 * @param {object} [benchmarks] - Override the WALLACE_2026 distribution
 * @returns {{metrics: object, overallPercentile: number, summary: string}}
 */
export function benchmarkAgainstWallace(metrics, benchmarks = WALLACE_2026) {
  const perMetric = {}
  const percentiles = []
  ...
  for (const key of Object.keys(benchmarks)) {
    ...
    const raw = valuePercentile(value, dist)
    const health = dist.lowerIsHealthier ? 100 - raw : raw
    let verdict
    if (health >= 75) verdict = 'better'
    else if (health >= 25) verdict = 'typical'
    else verdict = 'worse'

    percentiles.push(health)
    perMetric[key] = {
      value: round4(value),
      healthPercentile: round2(health),
      verdict,
    }
  }

  const overall = percentiles.length
    ? round2(percentiles.reduce((a, b) => a + b, 0) / percentiles.length)
    : 0

  let bucket
  if (overall >= 75)
    bucket = 'healthier than the vast majority of real-world CSS'
  else if (overall >= 50)
    bucket = 'healthier than a typical production stylesheet'
  else if (overall >= 25)
    bucket = 'less healthy than the median production stylesheet'
  else bucket = 'in the worst tier of real-world CSS complexity'

  return {
    metrics: perMetric,
    overallPercentile: overall,
    summary: `Overall health percentile ${overall}/100 — ${bucket} (Project Wallace 2026, >100k sites).`,
  }
}

// ---------------------------------------------------------------------------
// Milestone 4 — CLI health-score report (score breakdown per metric)
// ---------------------------------------------------------------------------

/**
 * Human-readable label for each metric, in report display order.
 * @type {Record<string, string>}
 */
export const METRIC_LABELS = {
  selectorsPerRule: 'Selectors per rule',
  declarationsPerRule: 'Declarations per rule',
  importantRatio: '!important ratio',
  avgSpecificity: 'Average specificity',
  layerAdoption: '@layer adoption',
}

function formatMetricValue(key, value) {
  if (key === 'importantRatio' || key === 'layerAdoption') {
...
/**
 * Build a structured health report from computed CSS metrics.
 *
 * Pure and unit-testable: no I/O, no ANSI styling. The CLI renders this for
 * humans; automated callers can read the returned object directly.
 *
 * @param {object} metrics - Output of computeMetrics
 * @returns {{score: number, metrics: Array<object>, benchmark: object}}
 */
export function buildHealthReport(
  ...
  const bench = benchmarkAgainstWallace(metrics)
  const breakdown = []
  let errors = 0
  let warnings = 0
  for (const key of Object.keys(METRIC_LABELS)) {
    const meta = bench.metrics[key] || {}
    const healthPercentile =
      meta.healthPercentile != null ? meta.healthPercentile : 0
    const severity = severityFor(healthPercentile, severityThresholds)
    if (severity === 'error') errors++
    else if (severity === 'warning') warnings++
    breakdown.push({
      key,
      label: METRIC_LABELS[key],
      value: formatMetricValue(key, Number(metrics?.[key]) || 0),
      healthPercentile,
      verdict: meta.verdict != null ? meta.verdict : 'typical',
      severity,
    })
  }
  const status = errors > 0 ? 'error' : warnings > 0 ? 'warning' : 'ok'
  const exitCode = status === 'error' ? 2 : status === 'warning' ? 1 : 0
  return {
    score,
    metrics: breakdown,
    status,
    ...
    benchmark: {
      ...
      overallPercentile: bench.overallPercentile,
...
/**
 * Render the health report as a plain-text string (no ANSI styling).
 *
 * @param {object} metrics - Output of computeMetrics
 * @returns {string}
 */
export function renderHealthReport(
  ...
  const report = buildHealthReport(metrics, severityThresholds)
  const lines = []
  lines.push('CSS Health Score: ' + report.score + '/100')
  lines.push('')
  lines.push('Per-metric breakdown:')
  ...
  lines.push('')
  lines.push(report.benchmark.summary)
  lines.push('')
  const summary =
    report.status === 'error'
      ? 'Status: ERROR — ' +
        report.errors +
        ' metric(s) below the error threshold.'
      : report.status === 'warning'
        ? 'Status: WARNING — ' +
          report.warnings +
          ' metric(s) below the warning threshold.'
        : 'Status: OK — all metrics within configured thresholds.'
  lines.push(summary)
  return lines.join('\n')
}
...
/**
 * Default severity thresholds, seeded from the Project Wallace 2026 verdict
 * buckets (better >= 75, typical 25-75, worse < 25). A metric's *health*
 * percentile (share of real-world sites at least as healthy) is compared
 * against these bounds: at or below `error` it is an error, at or below
 * `warning` it is a warning, otherwise it is within tolerance.
 *
 * Callers may override any subset via buildHealthReport(metrics, thresholds)
 * / renderHealthReport(metrics, thresholds). Thresholds are plain percentiles
 * in [0, 100], so they are trivially serializable to a config file.
 */
...
/**
 * Resolve a metric's severity from its health percentile.
 *
 * @param {number} healthPercentile - [0, 100]
 * @param {{error?: number, warning?: number}} [thresholds]
...
export function severityFor(
  ...
  const hp = Number(healthPercentile) || 0
  const errorAt = thresholds.error != null ? Number(thresholds.error) : 25
  const warnAt = thresholds.warning != null ? Number(thresholds.warning) : 50
  if (hp <= errorAt) return 'error'
  if (hp <= warnAt) return 'warning'
  return 'ok'
}
