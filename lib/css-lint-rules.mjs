...
/**
 * Scan CSS for `animation-timeline` declarations that rely on scroll-driven
 * animation timelines (`scroll()` and `view()`), and verify whether each one
 * carries a fallback for browsers without scroll-timeline support.
 *
 * The CSS Scroll-driven Animations spec introduces `animation-timeline` with
 * the anonymous `scroll()` and `view()` timeline functions, which lack wide
 * browser support and have no linting coverage in stylelint or @eslint/css.
 * This rule finds every `animation-timeline` declaration, classifies it by
 * timeline function, and records a fallback verdict:
 *
 * - `supports-guard` — the declaration sits inside an `@supports` block, so
 *   browsers without support skip it entirely.
 * - `time-based-animation` — the same rule also declares a time-based
 *   animation (a positive `animation-duration` or an `animation` shorthand
 *   with a positive duration), which runs as the fallback.
 * - `none` — no fallback detected; the declaration should be guarded.
 *
 * The return shape mirrors the `ScrollDrivenAnimationAudit` interface in
 * `lib/types.ts`.
 *
 * @param {string} css - Raw CSS source
 * @returns {{ totalAnimationTimelines: number, scrollTimelineCount: number, viewTimelineCount: number, missingFallbackCount: number, issues: Array<{ selector: string, property: string, value: string, timelineFunction: string, hasFallback: boolean, fallbackType: string, suggestion: string }> }}
 */
export function lintScrollDrivenAnimations(css) {
  const source = String(css)
    .replace(/\/\*[\s\S]*?\*\//g, ' ')
    .replace(/\/\/[^\n]*/g, ' ')

  // Collect rule blocks with brace tracking so nested media/container queries
  // are flattened and the enclosing selector is preserved for analysis. An
  // at-rule context stack records whether each rule sits inside an `@supports`
  // block, which is the signal for a supports-guard fallback.
  const rules = []
  const blockStack = []
  let selStart = -1
  for (let i = 0; i < source.length; i++) {
    ...
    if (ch === '{') {
      ...
      const inSupports = blockStack.some((s) => /^@supports\b/i.test(s))
      rules.push({ selector, body, inSupports })
      blockStack.push(selector)
      selStart = -1
    } else if (ch === '}') {
      blockStack.pop()
      selStart = -1
    } else if (selStart === -1 && /\S/.test(ch) && ch !== ';') {
      selStart = i
    }
  }

  const issues = []
  let totalAnimationTimelines = 0
  let scrollTimelineCount = 0
  let viewTimelineCount = 0
  const declRe = /([a-z-]+)\s*:\s*([^;]+)/gi

  // True when a value carries a positive CSS time token (e.g. "1s", "500ms").
  // Zero durations ("0s") do not count: a zero-length animation is not a
  // working fallback.
  const hasPositiveTime = (value) => {
    const timeRe = /(\d*\.?\d+)\s*(ms|s)\b/gi
    let m
    while ((m = timeRe.exec(value)) !== null) {
  ...
  for (const rule of rules) {
    ...
    while ((declMatch = declRe.exec(rule.body)) !== null) {
      declarations.push({
        prop: declMatch[1].trim().toLowerCase(),
        value: declMatch[2].trim(),
      })
    }

    // A time-based animation in the same rule is a fallback: browsers without
    // `animation-timeline` support ignore the timeline and run the time-based
    // animation instead.
    const hasTimeBasedFallback = declarations.some(
      (d) =>
        (d.prop === 'animation-duration' && hasPositiveTime(d.value)) ||
        (d.prop === 'animation' && hasPositiveTime(d.value))
    )

    for (const d of declarations) {
      if (d.prop !== 'animation-timeline') continue
      const value = d.value
      totalAnimationTimelines += 1

      let timelineFunction = 'other'
      if (/scroll\s*\(/i.test(value)) {
        timelineFunction = 'scroll()'
        scrollTimelineCount += 1
      } else if (/view\s*\(/i.test(value)) {
        timelineFunction = 'view()'
        viewTimelineCount += 1
      }

      const fallbackType = rule.inSupports
        ? 'supports-guard'
        : hasTimeBasedFallback
          ? 'time-based-animation'
          : 'none'

      let suggestion
      if (fallbackType === 'supports-guard') {
        suggestion =
          'Wrapped in an @supports guard; browsers without animation-timeline support will skip this declaration.'
      } else if (fallbackType === 'time-based-animation') {
        suggestion =
          'A time-based animation in the same rule provides a fallback for browsers without animation-timeline support.'
      } else {
        suggestion =
          'Provide a fallback for browsers without animation-timeline support: wrap the declaration in an @supports (animation-timeline: scroll()) guard, or add an equivalent time-based animation.'
      }
  ...
  return {
    totalAnimationTimelines,
    scrollTimelineCount,
    viewTimelineCount,
    missingFallbackCount: issues.filter((i) => !i.hasFallback).length,
    issues,
  }
}

/**
 * Build a coverage report for scroll-driven animations in a stylesheet.
 *
 * Aggregates the per-declaration scan from `lintScrollDrivenAnimations` into a
 * coverage summary: of every `animation-timeline` declaration, how many are
 * guarded by a fallback (an @supports wrapper or a time-based animation) and
 * how many are exposed to browsers without scroll-timeline support. Each
 * uncovered declaration becomes a warning with a concrete remediation step.
 *
 * @param {string} css - Raw CSS source
 * @returns {{ coverage: { total: number, withFallback: number, missingFallback: number, coverageRatio: number|null }, warnings: Array<{ selector: string, property: string, value: string, timelineFunction: string, message: string, suggestion: string }> }}
 */
export function lintScrollDrivenCoverage(css) {
  const result = lintScrollDrivenAnimations(css)
  const total = result.totalAnimationTimelines
  const missingFallback = result.missingFallbackCount
  const withFallback = total - missingFallback

  const warnings = result.issues
    .filter((i) => !i.hasFallback)
    .map((i) => ({
      selector: i.selector,
      property: i.property,
      value: i.value,
      timelineFunction: i.timelineFunction,
      message:
        `Scroll-driven animation '${i.timelineFunction}' on '${i.selector}' ` +
        'has no fallback for browsers without animation-timeline support.',
  ...
  return {
    coverage: {
      ...
      withFallback,
      missingFallback,
      coverageRatio: total === 0 ? null : withFallback / total,
    },
    warnings,
  }
}
