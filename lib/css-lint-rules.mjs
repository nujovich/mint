 ...
 * animation timelines (`scroll()` and `view()`).
 *
 ...
 * This rule finds every `animation-timeline` declaration and classifies it by
 * timeline function so later milestones can emit fallback and accessibility
 * warnings.
 *
 ...
 *
 * @param {string} css - Raw CSS source
 * @returns {{ totalAnimationTimelines: number, scrollTimelineCount: number, viewTimelineCount: number, issues: Array<{ selector: string, property: string, value: string, timelineFunction: string, suggestion: string }> }}
 */
export function lintScrollDrivenAnimations(css) {
  ...
  // are flattened and the enclosing selector is preserved for analysis.
  const rules = []
  let selStart = -1
  ...
  for (const rule of rules) {
    ...
    while ((declMatch = declRe.exec(rule.body)) !== null) {
      ...
      if (prop !== 'animation-timeline') continue
      ...
      issues.push({
        ...
        timelineFunction,
        suggestion:
          'Provide a fallback for browsers without animation-timeline support, e.g. an @supports guard or an equivalent static animation.',
      })
    }
  }
