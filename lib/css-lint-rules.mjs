...
import { detectColorSpace, toOklch } from './css-values.mjs'
...
// Color literal scanner for the OKLCH adoption audit. Matches hex colors and
// rgb()/rgba()/hsl()/hsla()/oklch()/oklab()/color() function literals anywhere
// in a declaration value (including inside gradients and shadows). Keyword
// colors (red, blue) and var()/color-mix() indirections are skipped — they are
// not statically classifiable literals.
const COLOR_LITERAL_RE =
  /#(?:[0-9a-fA-F]{3,8})\b|(?:rgba|rgb|hsla|hsl|oklch|oklab|color)\([^)]*\)/g

/**
 * Scan CSS source and classify every color literal by color space, producing
 * an OKLCH adoption report: how many colors use legacy sRGB formats (hex, rgb,
 * hsl) vs wide-gamut formats (oklch, oklab, display-p3), the wide-gamut
 * adoption rate, and the list of legacy colors that are OKLCH migration
 * candidates.
 *
 * @param {string} css - Raw CSS source
 * @returns {{ legacyCount: number, wideGamutCount: number, adoptionRate: number, samples: Array<{value: string, format: string, category: string, selector: string, oklch?: string}>, legacyCandidates: Array<{value: string, format: string, category: string, selector: string, oklch?: string}> }}
 */
export function auditColorSpaces(css) {
  const samples = []
  for (const rule of parseCssRules(css)) {
    const matches = rule.body.match(COLOR_LITERAL_RE) || []
    for (const value of matches) {
      const classified = detectColorSpace(value)
  ...
  const legacyCandidates = samples.filter((s) => s.category === 'legacy')
  const legacyCount = legacyCandidates.length
  const wideGamutCount = samples.length - legacyCount
  const adoptionRate =
    samples.length === 0 ? 0 : wideGamutCount / samples.length
