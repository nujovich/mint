...
function scanAtLayers(src) {
  ...
  while ((m = re.exec(src)) !== null) {
    ...
    if (delimM[1] === ';') {
      ...
      if (names.length > 0) {
        results.push({ names, isBlock: false })
      }
    } else if (delimM[1] === '{') {
      ...
      const names = nameStr
        ...
        .map((s) => s.trim())
        .filter((n) => RE_VALID_NAME.test(n))
      results.push({ names, isBlock: true, start: openBraceIdx, end: closeIdx })
 ...
 * @returns {string[]} selectors of rules outside layers (trimmed)
 */
...
export function lintCascadeLayers(css) {
  ...
  const unlayeredSelectors = findUnlayeredRules(src, layerSpans)

  return {
    layers,
    layerCount: layers.length,
    rulesOutsideLayers: unlayeredSelectors.length,
    issues: unlayeredSelectors.map((selector) => ({
      selector,
