...
/**
 * Detect declarations whose property name repeats within the same rule body,
 * so the earlier (dead) declaration is shadowed by the later one. Deliberate
 * fallback patterns are skipped: a vendor-prefixed value first (e.g.
 * `display: -webkit-box` before `display: flex`) or a literal value before a
 * `var()` reference.
 *
 * @param {string} selector - The rule's selector, for reporting.
 * @param {string} body - The rule body text.
 * @returns {Array<{selector, rule, severity, reason, property, deadValue, liveValue}>}
 */
function detectOverriddenDeclarations(selector, body) {
  const issues = []
  const declRe = /([a-z-]+)\s*:\s*([^;]+)/gi
  ...
  while ((m = declRe.exec(body)) !== null) {
    ...
    if (firstSeen.has(prop)) {
      const deadValue = firstSeen.get(prop)
      const isVendorFallback = deadValue.startsWith('-')
      const isVarFallback = /var\(/i.test(value)
      if (!isVendorFallback && !isVarFallback) {
        issues.push({
          selector,
          rule: 'overridden-property',
          severity: 'warning',
          reason:
            `Property "${prop}" is declared more than once; only the last ` +
            'declaration applies, so the earlier value is dead.',
          property: prop,
          deadValue,
          liveValue: value,
        })
    ...
    firstSeen.set(prop, value)
  }
  return issues
}

/**
 * A selector has zero specificity when it contributes no id, class, attribute,
 * pseudo-class, pseudo-element, or type weight — only universal selectors,
 * combinators, and `:where()` (which itself contributes zero).
 *
 * @param {string} selector - A raw CSS selector.
 * @returns {boolean}
 */
function isZeroSpecificitySelector(selector) {
  let s = selector
  // :where(...) contributes zero specificity — strip it (single nesting level).
  s = s.replace(/:where\([^()]*\)/g, ' ')
...
/**
 * Static dead-CSS estimator. Without a browser/DOM, this approximates
 * "non-applicable rules" with three deterministic signals:
 *
 *   - overridden declarations: a property declared twice in one rule (the
 *     earlier value is dead)
 *   - zero-specificity selectors: selectors whose specificity is (0,0,0)
 *   - duplicate selectors: the same top-level selector declared more than once
 *     (the earlier rule is shadowed in cascade order)
 *
 * The returned shape mirrors the `DeadCssAudit` interface in `lib/types.ts`.
 *
 * @param {string} css - Raw CSS source.
 * @returns {{ totalIssues: number, overriddenDeclarations: Array, zeroSpecificitySelectors: Array, duplicateSelectors: Array }}
 */
export function lintDeadCss(css) {
  ...
  for (let i = 0; i < source.length; i++) {
    ...
    if (ch === '{') {
      ...
      if (selector) rules.push({ selector, body, depth, isAtRule })
  ...
  const zeroSpecificitySelectors = []
  ...
  for (const rule of rules) {
    ...
    for (const issue of detectOverriddenDeclarations(
      rule.selector,
      rule.body
    )) {
      overriddenDeclarations.push(issue)
    }

    if (isZeroSpecificitySelector(rule.selector)) {
      zeroSpecificitySelectors.push({
        selector: rule.selector,
        rule: 'zero-specificity',
        severity: 'warning',
        reason:
          'Selector has specificity (0,0,0) — it matches only via the universal ' +
          'selector or :where(), so it is easily overridden and likely dead or accidental.',
      })
    }
    ...
    if (rule.depth === 0) {
      if (seenSelectors.has(rule.selector)) {
        duplicateSelectors.push({
          ...
          severity: 'suggestion',
          reason:
            `Selector "${rule.selector}" is declared more than once; the earlier ` +
            'rule is overridden by the later rule in cascade order, so its declarations may be dead.',
        })
      } else {
        seenSelectors.set(rule.selector, true)
      }
    }
  }

  const totalIssues =
    overriddenDeclarations.length +
    zeroSpecificitySelectors.length +
    duplicateSelectors.length

  return {
    totalIssues,
    overriddenDeclarations,
    zeroSpecificitySelectors,
    duplicateSelectors,
  }
}

/**
 * Render a plain-text report for a static dead-CSS scan. Used by the CLI's
 * `audit --dead-css` flag; kept dependency-free so it can be unit-tested.
 *
 * @param {object} deadCssAudit - The result of `lintDeadCss()`.
 * @param {number} [fileCount=1] - Number of stylesheets scanned, for context.
 * @returns {string} A multi-line, plain-text report.
 */
export function formatDeadCssReport(deadCssAudit, fileCount = 1) {
  const audit = deadCssAudit || {
    totalIssues: 0,
    overriddenDeclarations: [],
    zeroSpecificitySelectors: [],
    duplicateSelectors: [],
  ...
  const lines = []
  lines.push(
    `static scan across ${fileCount} file(s): ` +
      `${overriddenDeclarations.length} overridden declaration(s), ` +
      `${zeroSpecificitySelectors.length} zero-specificity selector(s), ` +
      `${duplicateSelectors.length} duplicate selector(s)`
  )

  if (audit.totalIssues === 0) {
    lines.push('No dead CSS found.')
    return lines.join('\n')
  }

  for (const issue of overriddenDeclarations) {
    lines.push('')
    lines.push(`WARN  ${issue.selector}`)
    lines.push(`      ${issue.reason}`)
    lines.push(`      dead: ${issue.deadValue}  ->  live: ${issue.liveValue}`)
  }
  for (const issue of zeroSpecificitySelectors) {
    lines.push('')
    lines.push(`WARN  ${issue.selector}`)
    lines.push(`      ${issue.reason}`)
  }
  for (const issue of duplicateSelectors) {
    lines.push('')
    lines.push(`INFO  ${issue.selector}`)
    lines.push(`      ${issue.reason}`)
  }
