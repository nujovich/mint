 ...
 * / `transition-property` declaration that sits outside such a wrapper and
 * emits a per-declaration issue with a wrap suggestion.
 *
...
export function lintMotionAccessibility(css) {
  ...
  for (const rule of rules) {
    if (rule.reduced || rule.inReduced || rule.isAtRule) continue
    declRe.lastIndex = 0
    let declMatch
